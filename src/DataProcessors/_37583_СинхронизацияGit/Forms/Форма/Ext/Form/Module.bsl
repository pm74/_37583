...
&НаКлиенте
Функция НовоеHTTPСоединение(АдресGitHubApi="api.github.com",Порт=443)
...
//
&НаКлиенте
Функция ДанныеЗапросаGitHub(HTTPСоединение,HTTPЗапрос,RAWReading=False)
	Ответ = HTTPСоединение.Получить(HTTPЗапрос) ;
	Данные = Неопределено;
	Если Ответ.КодСостояния = 200 Тогда 
		Результат = Ответ.ПолучитьТелоКакСтроку(); 
		Если RAWReading Тогда 
			Возврат Результат;
		КонецЕсли;
...
Процедура ПолучитьДанныеРесурсаGitHub(HTTPСоединение,АдресРесурса,ЭлементДерева)
	...
	HTTPЗапрос = Новый HTTPЗапрос ; 
	HTTPЗапрос.АдресРесурса = АдресРесурса; 
	ДанныеЗапроса = ДанныеЗапросаGitHub(HTTPСоединение,HTTPЗапрос,ИзвлекатьСтроковоеСодержимое);
...
Процедура ОбработатьДанныеЗапросаРесурсовGitHub(HTTPСоединение,ЭлементДерева)
	...
	Иначе
		Если ЭлементДерева.Пометка 
			...
			И ЭлементДерева.size < ОграничениеРазмераФайлов
			И ВыполняетсяУсловиеПоТипуФайлов(ЭлементДерева.Path) Тогда
			Если ИзвлекатьСтроковоеСодержимое Тогда 
				GitDownload = "https://raw.githubusercontent.com";
				АдресРесурса = СтрЗаменить(ЭлементДерева.download_url,GitDownload,"");
			Иначе
...
&НаКлиенте
Функция ВыполняетсяУсловиеПоТипуФайлов(СтрокаПутьФайла)
...
Процедура ОбработатьДанныеЗапросаРесурса(ДанныеЗапроса,HTTPСоединение,СтрокаДерева)
	...
	Если ТипЗнч(ДанныеЗапроса) = Тип("Строка") Тогда
		СтрокаДерева.content = ДанныеЗапроса;
...
Процедура ПрочитатьСодержимоеРепозиторияGit(Команда)
	...
	Для Каждого СтрокаРеп Из Объект.Репозитории Цикл
		Если СтрокаРеп.Использование Тогда
			...
			ЭлементРеп.РесурсGitHub = СтрокаРеп.АдресРесурса+"/contents"; 
			АдресРесурса = ПолучитьАдресРесурса(ЭлементРеп.РесурсGitHub,"");
			url = АдресРесурса + "?ref=" + ?(ПустаяСтрока(СтрокаРеп.Ветка),"master",СокрЛП(СтрокаРеп.Ветка));
			ЭлементРеп.name = url;
			ЭлементРеп.url = url;
			ЭлементРеп.Ссылка = СтрокаРеп.Каталог;
...
Процедура ПрочитатьСодержимоеПапки(Команда)
	...
	КонецЕсли;
	Элементы.ДеревоПакетовИзвлечьОтмеченныеРесурсы.Доступность = Истина;
КонецПроцедуры
...
Процедура ИзвлечьОтмеченныеРесурсы(Команда)
	Если ИзвлекатьСтроковоеСодержимое Тогда 
		СтрокаСоединения = "raw.githubusercontent.com";
	Иначе
		СтрокаСоединения = "api.github.com";
	КонецЕсли;
	HTTPСоединение = НовоеHTTPСоединение(СтрокаСоединения);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	|	Репозитории.ЛокальныйКаталог КАК Каталог,
	...
	// подключаемые команды Не БСП
	
	_37583_ПодключаемыеКомандыИОбработчики.ПриСозданииНаСервере(ЭтотОбъект);
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	Если Настройки["ОграничениеРазмераФайлов"] = Неопределено Тогда
		ОграничениеРазмераФайлов = 524288;
	КонецЕсли;
	Если Настройки["ИзвлекатьСтроковоеСодержимое"] = Неопределено Тогда
		ИзвлекатьСтроковоеСодержимое = Истина;
	КонецЕсли;
	Если Настройки["ОбрабатыватьТолькоФайлыСРаширениями"] = Неопределено 
		Или ПустаяСтрока(Настройки["ОбрабатыватьТолькоФайлыСРаширениями"])Тогда
		ОбрабатыватьТолькоФайлыСРаширениями = "xml,txt";
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ДеревоПакетовВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "Html_url" Тогда
		ПерейтиПоНавигационнойСсылке(Элемент.ТекущиеДанные.html_url);
	КонецЕсли;
КонецПроцедуры
...
#Область ПодключаемыеКоманды_37583_


&НаКлиенте
Процедура _37583_ВыполнитьКоманду(Команда) Экспорт
	
	_37583_АлгоритмыКлиент.ВыполнитьКоманду(ЭтотОбъект, Команда, Объект);
