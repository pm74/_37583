...
Процедура ВыполнитьКодАлгоритмаНаКлиенте(this,ПредставлениеАлгоритма,Параметры)
	...
	Если СвойстваАлгоритма.Отказ Тогда
		Возврат;
	КонецЕсли;
   ...
   Если  Не  СвойстваАлгоритма.Отказ Тогда
	   ...
	   //@Decorate
	   ПодключитьДекораторы(СвойстваАлгоритма, Параметры);
      
       Попытка
           Выполнить Параметры.КодАлгоритма;
           Если СвойстваАлгоритма.ЗаписыватьСобытиияВЖР  Тогда 
               _37583_АлгоритмыСервер.ЗаписатьВЖурналРегистрации("37583_ALG "," (&НаКлиенте) "+ СвойстваАлгоритма.Наименование,СвойстваАлгоритма.Ссылка, "Информация");
           КонецЕсли;
       Исключение
...
Процедура ПодключитьДекораторы(СвойстваАлгоритма,ПараметрыАлгоритма)
	Для Каждого Элемент Из СвойстваАлгоритма.Декораторы Цикл
		Если Элемент.Декоратор = СвойстваАлгоритма.Ссылка Или Элемент.Декоратор.Пустая() Тогда
			Продолжить;
		КонецЕсли;
		_37583_АлгоритмыКлиент.ВыполнитьПроцедуру(Элемент.Декоратор,Новый Структура("Параметры",ПараметрыАлгоритма));
	КонецЦикла;
КонецПроцедуры
