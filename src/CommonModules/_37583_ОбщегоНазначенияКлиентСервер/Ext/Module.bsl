...
Функция ПреобразоватьТекстВКодАлгоритма(Текст) Экспорт 
    ...
    пЗаменаВставка(КодАлгоритма,"__зф","_37583_АлгоритмыСервер.ВыполнитьФункцию","[""Результат""]",мИсключая);
	
	// возвращает  значение функции на клиенте __фзк($алгоритм,Новый Структура("",ЗначениеПараметра))
    пЗаменаВставка(КодАлгоритма,"__зфк","_37583_АлгоритмыКлиент.ВыполнитьФункцию","[""Результат""]",мИсключая);
	...
	КодАлгоритма = СтрЗаменить(КодАлгоритма,"__env","_37583_АлгоритмыКэш.ПолучитьПараметры_37583()");
...
Процедура ПодключитьДекораторыКода(Текст)
	...
	Пока Найден > 0 Цикл
		...
		УИД = СтрЗаменить(Декоратор,Имя,"");
		Суффикс =СтрНайти(Текст,"//"+УИД,,Начало);
		ТекстНаЗамену = СокрЛП(Сред(Текст,Начало,Суффикс-Начало-1));
		Текст = Лев(Текст,Найден-1)+ СтрЗаменить(Декоратор,"//@decorate_","//@decorated_")+Символы.ПС+ТекстНаЗамену+Символы.ПС+ СокрЛП(Сред(Текст,Суффикс));
