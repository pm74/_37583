...
Процедура УправлениеВидимостьюЭлементовФормы(П="",ВидимостьКнопок=Истина)
	Для Каждого Эл Из  Элементы Цикл
		Если ТипЗнч(Эл)=Тип("ДекорацияФормы") Тогда  Продолжить; КонецЕсли;
...
Функция КоллекцияВТЗ(Коллекция) 
	...
	Если Не ТипЗнч(Коллекция)=Тип("Массив")Тогда 
		...
		КонецЦикла;
	Иначе 
		Для I=0 по Коллекция.ВГраница()Цикл ТЗ.Добавить();КонецЦикла; ТЗ.ЗагрузитьКолонку(Коллекция,0);
	КонецЕсли;
	Возврат ТЗ;
КонецФункции
...
Процедура ДобавитьКолонку(Команда)
	КолонкаИмя="";
	ПоказатьВводЗначения(Новый ОписаниеОповещения("ДобавитьКолонкуЗавершение", ЭтаФорма, Новый Структура("КолонкаИмя", КолонкаИмя)), КолонкаИмя,"Введите имя новой колонки","Строка");
КонецПроцедуры
...
Процедура УдалитьКолонку(Команда)
	...
	Если Элементы.ПараметрКоллекция.ТекущийЭлемент <> Неопределено Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("УдалитьКолонкуЗавершение", ЭтаФорма, Новый Структура("ИмяКолонки", ИмяКолонки)), "Вы уверены что хотите изменить удалить колонку  """ +ИмяКолонки+""" ?", РежимДиалогаВопрос.ДаНет); 
	Иначе
...
Процедура ИзменитьПараметр()
	...
	Если Не НовоеЗначение=Неопределено Тогда 
		ВыбОбъект=РеквизитФормыВЗначение("Объект");
		ВыбОбъект.ИзменитьПараметр(Новый Структура("НаименованиеПараметра,ЗначениеПараметра",НаименованиеПараметра,НовоеЗначение));
	КонецЕсли;
КонецПроцедуры
...
Функция ПолучитьНовоеЗначение()
	...
	Иначе
		...
		ИначеЕсли ТипКоллекция="Структура" Тогда
			С=Новый Структура;
			Для каждого Стр Из Т Цикл  С.Вставить(Стр.Ключ,Стр.Значение)КонецЦикла ;
			Возврат С;
...
&НаКлиенте
Процедура ОпределяемыйТипОкончаниеВводаТекста(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, СтандартнаяОбработка)
